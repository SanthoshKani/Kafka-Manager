...
public class KafkaAdminClientConfiguration {
    ...
    Admin kafkaAdminClient(KafkaManagerProperties properties) {
        ...
        adminClientProperties.put(
                AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, KafkaEndpointSupport.normalizeEndpointList(admin.bootstrapServers()));
        ...
        adminClientProperties.put(
                AdminClientConfig.DEFAULT_API_TIMEOUT_MS_CONFIG, Math.toIntExact(admin.defaultOperationTimeout().toMillis()));
        adminClientProperties.put(CommonClientConfigs.CONNECTIONS_MAX_IDLE_MS_CONFIG, 300_000L);
        adminClientProperties.put(
                CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, KafkaEndpointSupport.normalizeSecurityProtocol(admin.securityProtocol()));
